...
import 'package:news_api_task/Entertainment_News_API/entnewsbloc/ent_api_bloc.dart';
...
class EntNewsPage extends StatefulWidget {
  const EntNewsPage({super.key});

  @override
  State<EntNewsPage> createState() => _EntNewsPageState();
}

class _EntNewsPageState extends State<EntNewsPage> {
  EntApiBloc entApiBloc = EntApiBloc();

  @override
  void initState() {
    entApiBloc.add(EntApiData());
    super.initState();
  }
          ...
          child: Column(
            children: [
              Container(
                ...
                child: BlocProvider(
                  create: (context) => entApiBloc,
                  child: BlocBuilder<EntApiBloc, EntApiState>(
                    builder: (context, state) {
                      if (state is EntApiLoading) {
                        // Display CircularProgressIndicator while loading
                        return Center(
                          child: CircularProgressIndicator(color: Color(0xffef5252),),
                        );
                      }
                      else if (state is EntApiLoaded) {
                        return ListView.builder(
                            physics: NeverScrollableScrollPhysics(),
                            shrinkWrap: true,
                            itemCount: state.entNewsModel.articles?.length,
                            itemBuilder: (BuildContext context, int index) {
                              final results = state.entNewsModel.articles;
