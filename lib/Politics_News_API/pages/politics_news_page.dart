...
import 'package:news_api_task/Politics_News_API/politicsnewsbloc/politics_api_bloc.dart';
...
class PoliticsNewsPage extends StatefulWidget {
  const PoliticsNewsPage({super.key});

  @override
  State<PoliticsNewsPage> createState() => _PoliticsNewsPageState();
}

class _PoliticsNewsPageState extends State<PoliticsNewsPage> {
PoliticsApiBloc _politicsApiBloc = PoliticsApiBloc();

  @override
  void initState() {
    super.initState();
    _politicsApiBloc.add(PoliticsApiData());
    print(PoliticsApiBloc);
  }
          ...
          child: Column(
            children: [
              Container(
                ...
                child: BlocProvider(
                  create: (context) => _politicsApiBloc,
                  child: BlocBuilder<PoliticsApiBloc, PoliticsApiState>(
                    builder: (context, state) {
                      if (state is PoliticsApiLoading) {
                        // Display CircularProgressIndicator while loading
                        return Center(
                          child: CircularProgressIndicator(color: Color(0xffef5252),),
                        );
                      }
                      else if (state is PoliticsApiLoaded) {
                        return ListView.builder(
                            physics: NeverScrollableScrollPhysics(),
                            shrinkWrap: true,
                            itemCount: state.politicsNewsModel.articles?.length,
                            itemBuilder: (BuildContext context, int index) {
                              final results = state.politicsNewsModel.articles;
                              if (results != null && results.length > index) {
                                final result = results[index];
                                final urlToImage = result.urlToImage?? '';
                                final title = result.title?? '';
                                final description = result.description?? '';
                                final publishedAt = result.publishedAt?? '';
                                final name = result.source?.name?? '';//s name
