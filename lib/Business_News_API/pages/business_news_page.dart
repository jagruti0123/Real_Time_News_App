...
import 'package:news_api_task/Business_News_API/businessnewsbloc/business_api_bloc.dart';
...
class _BusinessNewsPageState extends State<BusinessNewsPage> {
  BusinessApiBloc businessApiBloc = BusinessApiBloc();

  @override
  void initState() {
    businessApiBloc.add(BusinessApiData());
    super.initState();
  }
          ...
          child: Column(
            children: [
              Container(
                ...
                child: BlocProvider(
                  create: (context) => businessApiBloc,
                  child: BlocBuilder<BusinessApiBloc, BusinessApiState>(
                    builder: (context, state) {
                      if (state is BusinessApiLoading) {
                        // Display CircularProgressIndicator while loading
                        return Center(
                          child: CircularProgressIndicator(color: Color(0xffef5252),),
                        );
                      }
                      else if (state is BusinessApiLoaded) {
                        return ListView.builder(
                            physics: NeverScrollableScrollPhysics(),
                            shrinkWrap: true,
                            itemCount: state.businessNewsModel.articles?.length,
                            itemBuilder: (BuildContext context, int index) {
                              final results = state.businessNewsModel.articles;
