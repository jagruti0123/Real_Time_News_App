...
import 'package:news_api_task/Science_News_API/sciencenewsbloc/science_api_bloc.dart';
...
class _ScienceNewsPageState extends State<ScienceNewsPage> {
  ScienceApiBloc scienceApiBloc = ScienceApiBloc();

  @override
  void initState() {
    scienceApiBloc.add(ScienceApiData());
    super.initState();
  }
          ...
          child: Column(
            children: [
              Container(
                ...
                child: BlocProvider(
                  create: (context) => scienceApiBloc,
                  child: BlocBuilder<ScienceApiBloc, ScienceApiState>(
                    builder: (context, state) {
                      if (state is ScienceApiLoading) {
                        // Display CircularProgressIndicator while loading
                        return Center(
                          child: CircularProgressIndicator(color: Color(0xffef5252),),
                        );
                      }
                      else if (state is ScienceApiLoaded) {
                        return ListView.builder(
                            physics: NeverScrollableScrollPhysics(),
                            shrinkWrap: true,
                            itemCount: state.scienceNewsModel.articles?.length,
                            itemBuilder: (BuildContext context, int index) {
                              final results = state.scienceNewsModel.articles;
