...
import 'package:news_api_task/Edu_News_API/edunewsbloc/edu_api_bloc.dart';
...
class EduNewsPage extends StatefulWidget {
  const EduNewsPage({super.key});

  @override
  State<EduNewsPage> createState() => _EduNewsPageState();
}

class _EduNewsPageState extends State<EduNewsPage> {

  EduApiBloc eduApiBloc = EduApiBloc();

  @override
  void initState() {
    eduApiBloc.add(EduApiData());
    super.initState();
  }
          ...
          child: Column(
            children: [
              Container(
                ...
                child: BlocProvider(
                  create: (context) => eduApiBloc,
                  child: BlocBuilder<EduApiBloc, EduApiState>(
                    builder: (context, state) {
                      if (state is EduApiLoading) {
                        // Display CircularProgressIndicator while loading
                        return Center(
                          child: CircularProgressIndicator(color: Color(0xffef5252),),
                        );
                      }
                      else if (state is EduApiLoaded) {
                        return ListView.builder(
                            physics: NeverScrollableScrollPhysics(),
                            shrinkWrap: true,
                            itemCount: state.eduNewsModel.articles?.length,
                            itemBuilder: (BuildContext context, int index) {
                              final results = state.eduNewsModel.articles;
